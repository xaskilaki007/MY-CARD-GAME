...
double Alfa = 0; double T0 = 3000; double DT = 100; double Y0 = 0.6; double Y1 = 0.63;  double Cx = -0.45, Cy = 0, Cy2=-0.1;
const int nFig = 12;
double moveT[nFig];
int figType[nFig];
...
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("ШПАКОВ ФИНАЛЬНАЯ АНИМАЦИЯ");
...
void SwordsmansCard(double t, double x, double y, Figure* f) {
	...
	if(g)
		...
		glTranslated(0.22 + c * (x - 0.22), Cy2 + c * (y - Cy2), 0);
	if (!g)

		glRotated(180 * c + 180, 0, 1, 0);
...
void ArchmansCard(double t, double x, double y, Figure* f){
	...
	if(g)
		...
		glTranslated(0.2 + c * (x - 0.2), Cy2 + c * (y - Cy2), 0);
	if (!g)
		glRotated(180 * c + 180, 0, 1, 0);
	glRotated(180 * c + 180, 1, 0, 0);
	glScaled(0.6, 0.6, 0);//Это для уменьшения персонажа
...
void RiderCard(double t, double x, double y, Figure* f) {
	...
	if(g)
		...
		glTranslated(0.24 + c * (x - 0.24), Cy2 + c * (y - Cy2), 0);
	if(!g)
